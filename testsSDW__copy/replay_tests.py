...
from SDWLE.replay import Replay, record, playback
from SDWLE.agents.basic_agents import PredictableAgent, RandomAgent
from SDWLE.constants import CHARACTER_CLASS
from SDWLE.cards import *
import SDWLE.game_objects
from testsSDW.agents.testing_agents import PlayAndAttackAgent, OneCardPlayingAgent
from testsSDW.testing_utils import StackedDeck
...
class TestReplay(unittest.TestCase):
    ...
    def test_reading_and_writing_compact(self):
        ...
        get_files_from("testsSDW/replays/compact")
    ...
    def test_loading_game(self):
        game = playback(Replay("testsSDW/replays/example.hsreplay"))
    ...
    def test_recording_game(self):
        self.maxDiff = None
        random.seed(9876)
        deck1 = SDWLE.engine.Deck([StonetuskBoar() for i in range(0, 30)], Jaina())
        deck2 = SDWLE.engine.Deck([Naturalize() for i in range(0, 30)], Malfurion())
        ...
        f = open("testsSDW/replays/stonetusk_innervate.hsreplay", 'r')
        dif = self.__compare_json(output.getvalue(), f.read())
        self.assertTrue(dif)
        f.close()

    def test_option_replay(self):
        game = playback(Replay("testsSDW/replays/stonetusk_power.hsreplay"))
        game.start()
    ...
    def test_random_character_saving(self):
        deck1 = SDWLE.engine.Deck([RagnarosTheFirelord() for i in range(0, 30)], Jaina())
        deck2 = SDWLE.engine.Deck([StonetuskBoar() for i in range(0, 30)], Malfurion())
    ...
    def test_replay_validation(self):
        ...
        with open("replay.schema.json", "r") as schema_file:
            ...
            get_files_from("testsSDW/replays")
            for rfile in files:
