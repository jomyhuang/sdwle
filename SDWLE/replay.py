...
from SDWLE.engine import Game, card_lookup, Deck
import SDWLE.game_objects
import SDWLE.cards
import SDWLE.proxies
from SDWLE.serialization.move import Move, AttackMove, PowerMove, TurnEndMove, \
...
class Replay:
    ...
    def _record_card_played(self, card, index):
        ...
        """
        self._moves.append(PlayMove(SDWLE.proxies.ProxyCard(index), target=card.target))
        ...
        for line in file:
            ...
            if move == 'play':
                ...
                if len(args) > 1:
                    ...
                    target = None
                self._moves.append(PlayMove(SDWLE.proxies.ProxyCard(card), target=target))
                ...
                self._moves.append(PlayMove(SDWLE.proxies.ProxyCard(card), index, target))
            elif move == 'attack':
                ...
                if len(self._moves) == 0:
                    ...
                    for num in args:
                        if num.isdigit():
                            ...
                            self._moves[-1].random_numbers.append(SDWLE.proxies.ProxyCharacter(num))
...
def record(game):
    ...
    def random_choice(choice):
        ...
        if isinstance(result, SDWLE.game_objects.Character):
            replay._moves[-1].random_numbers[-1] = SDWLE.proxies.ProxyCharacter(result)
        return result
...
def playback(replay):
    ...
    class ReplayAgent:
        ...
        def do_turn(self, player):
            ...
            while move_index < len(replay._moves) and not player.hero.dead and type(
                    replay._moves[move_index]) is not SDWLE.serialization.move.TurnEndMove:
                random_index = 0
    ...
    def random_choice(choice):
        nonlocal move_index, random_index
        if isinstance(replay._moves[move_index].random_numbers[random_index], SDWLE.proxies.ProxyCharacter):
