    ...
    def from_json(name, random=[], **json):
        ...
        for num in random:
            if isinstance(num, dict):
                obj.random_numbers.append(SDWLE.proxies.ProxyCharacter.from_json(**num))
            else:
...
class PlayMove(Move):
    def __init__(self, card, index=-1, target=None):
        ...
        if target is not None:
            self.target = SDWLE.proxies.ProxyCharacter(target)
        else:
            self.target = None
    ...
    def __from_json__(self, card, index=-1, target=None):
        self.card = SDWLE.proxies.ProxyCard.from_json(**card)
        self.index = index
        if target:
            self.target = SDWLE.proxies.ProxyCharacter.from_json(**target)
        else:
            self.target = None
...
class AttackMove(Move):
    def __init__(self, character, target):
        super().__init__()
        self.character = SDWLE.proxies.ProxyCharacter(character)
        self.target = SDWLE.proxies.ProxyCharacter(target)
    ...
    def __from_json__(self, character, target):
        self.character = SDWLE.proxies.ProxyCharacter.from_json(**character)
        self.target = SDWLE.proxies.ProxyCharacter.from_json(**target)
