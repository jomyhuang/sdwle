...
from SDWLE.game_objects import Minion, Hero
...
from SDWLE.tags.status import Immune, ManaChange
...
class Flare(SpellCard):
    ...
    def use(self, player, game):
        super().use(player, game)
        for minion in SDWLE.targeting.find_minion_spell_target(game, lambda m: m.stealth):
...
class KillCommand(SpellCard):
    ...
    def use(self, player, game):
        super().use(player, game)
        beasts = SDWLE.targeting.find_friendly_minion_battlecry_target(
...
class UnleashTheHounds(SpellCard):
    ...
    def use(self, player, game):
        ...
        for target in SDWLE.targeting.find_enemy_minion_spell_target(player.game, lambda x: True):
            hound = SDWLE.cards.minions.hunter.Hound()
...
class AnimalCompanion(SpellCard):
    ...
    def use(self, player, game):
        ...
        beast_list = [SDWLE.cards.minions.hunter.Huffer(), SDWLE.cards.minions.hunter.Misha(),
                      SDWLE.cards.minions.hunter.Leokk()]
...
class SnakeTrap(SecretCard):
    ...
    def _reveal(self, attacker, target):
        if isinstance(target, Minion) and len(target.player.game.other_player.minions) < 7:
            snake = SDWLE.cards.minions.hunter.Snake()
...
class BearTrap(SecretCard):
    ...
    def _reveal(self, attacker, target):
        if isinstance(target, Hero) and len(target.player.game.other_player.minions) < 7:
            bear = SDWLE.cards.minions.neutral.IronfurGrizzly()
            player = target.player.game.other_player
