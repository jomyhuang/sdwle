import copy
from SDWLE.cards.base import ChoiceCard, SpellCard
from SDWLE.tags.action import Summon
from SDWLE.tags.base import Deathrattle
from SDWLE.tags.selector import PlayerSelector
...
class SummonPanther(ChoiceCard):
    ...
    def use(self, player, game):
        from SDWLE.cards.minions.druid import Panther
        panther = Panther()
...
class SoulOfTheForest(SpellCard):
    ...
    def use(self, player, game):
        super().use(player, game)
        from SDWLE.cards.minions.druid import Treant
...
class Swipe(SpellCard):
    def __init__(self):
        super().__init__("Swipe", 4, CHARACTER_CLASS.DRUID, CARD_RARITY.COMMON,
                         target_func=SDWLE.targeting.find_enemy_spell_target)
...
class DamageOne(ChoiceCard):
            ...
            def can_use(self, player, game):
                return super().can_use(player, game) and len(SDWLE.targeting.find_minion_spell_target(
                    game, lambda t: t.spell_targetable())) > 0

            def use(self, player, game):
                targets = SDWLE.targeting.find_minion_spell_target(game, lambda t: t.spell_targetable())
...
class PoisonSeeds(SpellCard):
    ...
    def use(self, player, game):
        super().use(player, game)
        from SDWLE.cards.minions.druid import PoisonSeedsTreant
        targets = SDWLE.targeting.find_minion_spell_target(game, lambda m: True)
...
class DarkWispers(SpellCard):
    ...
    def can_use(self, player, game):
        return (super().can_use(player, game) and
                (len(player.minions) < 7 or
                 SDWLE.targeting.find_minion_spell_target(game, lambda t: t.spell_targetable()) is not None))
    ...
    def use(self, player, game):
        ...
        class Buff5(ChoiceCard):
            ...
            def can_use(self, player, game):
                return SDWLE.targeting.find_minion_spell_target(game,
        ...
        class Wisps5(ChoiceCard):
            ...
            def use(self, player, game):
                from SDWLE.cards.minions.neutral import Wisp
                for i in range(0, 5):
                    wisp = Wisp()
                    wisp.summon(player, game, len(player.minions))

        # In the official interface, both options are shown, but only one is highlighted.
        if len(SDWLE.targeting.find_minion_spell_target(game, lambda t: t.spell_targetable())) == 0:
            option = Wisps5()
        else:
