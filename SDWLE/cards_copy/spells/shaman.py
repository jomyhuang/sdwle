...
from SDWLE.tags.base import Deathrattle, Buff
from SDWLE.tags.selector import PlayerSelector
from SDWLE.tags.status import Windfury as _Windfury, Frozen, ManaChange
...
class FeralSpirit(SpellCard):
    ...
    def use(self, player, game):
        ...
        for i in range(0, 2):
            spirit_wolf = SDWLE.cards.minions.shaman.SpiritWolf()
...
class Hex(SpellCard):
    ...
    def use(self, player, game):
        ...
        frog = SDWLE.cards.minions.neutral.Frog()
...
class LavaBurst(SpellCard):
    def __init__(self):
        super().__init__("Lava Burst", 3, CHARACTER_CLASS.SHAMAN, CARD_RARITY.RARE,
                         target_func=SDWLE.targeting.find_spell_target, overload=2)
...
class LightningBolt(SpellCard):
    def __init__(self):
        super().__init__("Lightning Bolt", 1, CHARACTER_CLASS.SHAMAN, CARD_RARITY.COMMON,
                         target_func=SDWLE.targeting.find_spell_target, overload=1)
...
class RockbiterWeapon(SpellCard):
    def __init__(self):
        super().__init__("Rockbiter Weapon", 1, CHARACTER_CLASS.SHAMAN, CARD_RARITY.FREE,
                         target_func=SDWLE.targeting.find_friendly_spell_target)
