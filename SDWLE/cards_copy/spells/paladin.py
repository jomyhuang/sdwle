import copy
from SDWLE.cards.base import SpellCard, SecretCard
from SDWLE.tags.action import Draw
from SDWLE.tags.base import Effect, Buff, ActionTag
from SDWLE.tags.event import Attack
from SDWLE.tags.selector import PlayerSelector, PlayerOne, PlayerTwo, BothPlayer, Count, DeadMinionSelector
from SDWLE.tags.status import DoubleAttack, ManaChange
import SDWLE.targeting
from SDWLE.constants import CHARACTER_CLASS, CARD_RARITY
from SDWLE.cards.minions.paladin import SilverHandRecruit
from SDWLE.cards.weapons.paladin import LightsJustice
...
class HolyLight(SpellCard):
    def __init__(self):
        super().__init__("Holy Light", 2, CHARACTER_CLASS.PALADIN,
                         CARD_RARITY.FREE, target_func=SDWLE.targeting.find_spell_target)
...
class HolyWrath(SpellCard):
    def __init__(self):
        super().__init__("Holy Wrath", 5, CHARACTER_CLASS.PALADIN,
                         CARD_RARITY.RARE, target_func=SDWLE.targeting.find_spell_target)
...
class NobleSacrifice(SecretCard):
    ...
    def _reveal(self, attacker, target):
        ...
        if len(player.minions) < 7 and not attacker.removed:
            from SDWLE.cards.minions.paladin import DefenderMinion
