import abc
from SDWLE.tags.base import Selector, Player, Picker, Function, Amount, Condition
import SDWLE.tags.condition
...
class MinionSelector(Selector):
    def __init__(self, condition=SDWLE.tags.condition.MinionIsNotTarget(), players=FriendlyPlayer(),
                 picker=AllPicker()):
        self.condition = condition
        self.players = players
        self.picker = picker
    ...
    def __from_json__(self, players, picker, condition=None):
        if condition:
            self.condition = SDWLE.tags.condition.Condition.from_json(**condition)
        else:
            self.condition = None
        self.players = Player.from_json(players)
        self.picker = Picker.from_json(**picker)
