...
class Player(metaclass=abc.ABCMeta):
    ...
    @staticmethod
    def from_json(name):
        from SDWLE.tags.selector import FriendlyPlayer, EnemyPlayer, BothPlayer, PlayerOne, \
...
class Picker(JSONObject, metaclass=abc.ABCMeta):
    ...
    def from_json(name, count=0):
        from SDWLE.tags.selector import UserPicker, AllPicker, RandomPicker
        if name == "user":
            return UserPicker()
        elif name == "all":
            return AllPicker()
...
class Selector(JSONObject, metaclass=abc.ABCMeta):
    ...
    def from_json(name, **kwargs):
        import SDWLE.tags.selector as selector_mod
...
class Action(JSONObject, metaclass=abc.ABCMeta):
    ...
    @staticmethod
    def from_json(name, **kwargs):
        import SDWLE.tags.action as action_mod
...
class Status(JSONObject, metaclass=abc.ABCMeta):
    ...
    @staticmethod
    def from_json(name, **kwargs):
        import SDWLE.tags.status as status_mod
...
class Event(JSONObject, metaclass=abc.ABCMeta):
    ...
    def from_json(event_name, **kwargs):
        import SDWLE.tags.event as event_mod
...
class Condition(JSONObject, metaclass=abc.ABCMeta):
    ...
    @staticmethod
    def from_json(name, **kwargs):
        import SDWLE.tags.condition as action_mod
...
class CardQuery(JSONObject, metaclass=abc.ABCMeta):
    ...
    @staticmethod
    def from_json(query):
        from SDWLE.tags.card_source import SpecificCard, CardList, HandSource, \
