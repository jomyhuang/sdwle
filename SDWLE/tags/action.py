...
from SDWLE.tags.base import Status, Action, Aura, Condition, AuraUntil, CardQuery, Effect, Buff, BuffUntil, \
    Amount, Picker, Selector, ActionTag
from SDWLE.tags.card_source import HandSource, SpecificCard
from SDWLE.tags.selector import AllPicker, ConstantSelector, EnemyPlayer, SelfSelector
...
class ChangeTarget(Action):
    ...
    def __from_json__(self, selector):
        from SDWLE.tags.base import Selector
...
class RemoveSecret(Action):
    ...
    def __from_json__(self, player):
        from SDWLE.tags.selector import Player
...
class RemoveDivineShields(Action):
    def act(self, actor, target, other=None):
        from SDWLE.tags.status import DivineShield
...
class SwapStats(Action):
    ...
    def set_attribute(obj, attribute, value):
        from SDWLE.tags.status import ManaChange, SetAttack
        if attribute == "damage":
