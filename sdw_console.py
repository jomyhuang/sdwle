...
def print_usage():
    usage = """usage: python sdw_console.py deck1 deck2
...
def console_input_index(targets=[], prompt='>'):
    selected = -1
    while selected < 0:
        s = input(prompt)
        if s is 'q':
...
def render_game():
    ...
    def choose_agent(window):
        ...
        return registry.create_agent(agents[0])
    ...
    if game.first_player == 0:
        ...
        renderer = ConsoleGameRender(stdscr, game, game.players[1])
    game.start()
